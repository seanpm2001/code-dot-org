...
export default function Congrats(props) {
  ...
  const renderExtraCertificateLinks = (language, tutorial, currentDate) => {
    const {extraLinkUrl, extraLinkText} = getExtraLinkData(language, tutorial, currentDate);
  ...
  const getExtraLinkData = (language, tutorial, currentDate) => {
    ...
    const codingPartyActive = codingPartyStart <= currentDate && currentDate < codingPartyEnd;
    if (language === 'ko' && codingPartyActive) {
      const extraLinkText = '온라인 코딩 파티 인증서 받으러 가기! (과학기술정보통신부 인증)';
  ...
  const currentDateOverride = hasQueryParam('currentDate') && new Date(queryParams('currentDate'));
